...
class IRBackbone(nn.Module):
    def __init__(self, model_name, ir_checkpoint, return_interm_layers: bool, in_channels=3):
        super().__init__()
        self.name = model_name 
        ...
        if 'resnet' in self.name.lower():
            ...
            if self.in_channels != 3:
                self.backbone.conv1 = nn.Conv2d(self.in_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)
            ...
            self.backbone = timm.create_model(
                ...
                pretrained_cfg_overlay=dict(file=ir_checkpoint)
            )
            self.data_config = timm.data.resolve_model_data_config(self.backbone)

            # # ...
            # # 如果你有红外专用的预训练权重（如某些自监督学习在红外数据集上训练的结果），在此加载
            # if ir_checkpoint:
            #     self.backbone.load_state_dict(torch.load(ir_checkpoint), strict=False)
            # # ...
...
class TimmModel(nn.Module):
    def __init__(self, sat_model_name,
        ...
        #----------------------- local -----------------------# 
        self.num_channles = self.backbone_sat.num_channels.copy()
    ...
    def get_config(self,):
        data_config = self.backbone_sat.data_config
        return data_config
    ...
    def forward(self, img1, img2=None, input_id=0):
        if img2 is not None:
            ...
            # 分别使用独立的 backbone 提取特征
            sat_x = self.backbone_sat(img2)  # img2 是 RGB 卫星图
            grd_x = self.backbone_grd(img1)  # img1 是 IR 无人机图

            # 分别使用独立的 embed 模块
            sat_e, sat_src = self.embed_sat(sat_x)
            grd_e, grd_src = self.embed_grd(grd_x)
...
class TimmModel_u(nn.Module):
    def __init__(self, model_name,
        ...
        # 2. 无人机流 (IR) Backbone 和 Embed
        self.backbone_grd = IRBackbone(self.backbone_name, return_interm_layers=not self.single_features, in_channels=1, ir_checkpoint=None)
        self.embed_grd = BackboneEmbed(self.d_model, self.backbone_grd.strides, self.backbone_grd.num_channels, return_interm_layers=not self.single_features)
        ...
        out_dim_g = 14
        self.feat_dim, self.H, self.W = self._dim(self.backbone_name, self.backbone_sat.strides, img_size=self.img_size)
        in_dim = sum(self.feat_dim[1:]) +  self.in_dim_L if self.sample else sum(self.feat_dim)
        self.proj = nn.Linear(in_dim, out_dim_g)


        #----------------------- local -----------------------# 
        self.num_channles = self.backbone_sat.num_channels
        self.num_channles.append(self.d_model)
